...
f=[10,10]%下次故障时间，等价于[f(t),fs(t)]
gc=[10,10];%维修时间，等价于[gc(t),gcs(t)]
gp=[10,10];%维护时间，等价于[gp(t),gps(t)]
...
ns_full=zeros(1,2);%是否满容，和status类似
%需要计算的结果
CMC=0;%累计维修费
CMP=0;%累计维护费
CPROD=0;%累计生产成本
...
while(tsim<=H)
    ...
    if(cur_machine~=3)
        ...
        if(cur_machine==1 && status(1)==1 && (cur_event==1 || cur_event==3) && status(2)==4)
        ...
        if(cur_machine==2 && status(2)==1 && status(1)==1)
